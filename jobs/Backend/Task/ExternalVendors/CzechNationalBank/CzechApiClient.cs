    ...
    public class CzechApiClient : IExchangeRateClient
    {
        ...
        public async Task<ExchangeRateDto> GetDailyExchangeRates()
        {
            ...
            try
            {
                ...
                return exchangeRates ?? ExchangeRateDto.Empty;
            }
            catch (HttpRequestException ex)
            {
                _logger.LogError(ex, "Failed to retrieve daily exchange rates from API. Response code: {StatusCode}",
                    ex.StatusCode);
                return ExchangeRateDto.Empty;
            }
        }
    }
}