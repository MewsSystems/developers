...
public class ActivateProviderCommandTests : IntegrationTestBase
{
    ...
    [Fact]
    public async Task ActivateProvider_WhenProviderIsDeactivated_ShouldActivateProvider()
    {
        // Arrange
        ...
        var activateCommand = new ActivateProviderCommand(providerId);
    ...
    [Fact]
    public async Task ActivateProvider_WhenProviderIsAlreadyActive_ShouldSucceed()
    {
        // Arrange - Provider is active by default when created
        var currencyId = await CreateTestCurrencyAsync();
        var providerId = await CreateTestProviderAsync(currencyId);

        // Act - Try to activate already active provider
        var command = new ActivateProviderCommand(providerId);
        var result = await Mediator.Send(command);
    ...
    [Fact]
    public async Task ActivateProvider_WithNonExistentProvider_ShouldFail()
    {
        // Arrange
        var command = new ActivateProviderCommand(99999);
    ...
    [Fact]
    public async Task ActivateProvider_MultipleProviders_ShouldActivateAll()
    {
        // Arrange
        var currencyId = await CreateTestCurrencyAsync();
        ...
        await Mediator.Send(new DeactivateProviderCommand(provider1Id));
        await Mediator.Send(new DeactivateProviderCommand(provider2Id));
        await Mediator.Send(new DeactivateProviderCommand(provider3Id));

        // Act - Reactivate all providers
        var result1 = await Mediator.Send(new ActivateProviderCommand(provider1Id));
        var result2 = await Mediator.Send(new ActivateProviderCommand(provider2Id));
        var result3 = await Mediator.Send(new ActivateProviderCommand(provider3Id));
