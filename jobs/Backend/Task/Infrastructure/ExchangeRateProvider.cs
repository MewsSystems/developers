...
namespace ExchangeRateUpdater.Infrastructure
{
    ...
    public class CzechNationalBankExchangeRateProvider : IExchangeRateProvider
    {
        ...
        private readonly IValidator<CnbRateDto> _rateDtoValidator;
        private readonly IExchangeRateMapper _mapper;
        private readonly IOptions<ExchangeRateProviderOptions> _options;

        public CzechNationalBankExchangeRateProvider(HttpClient httpClient, IValidator<CnbRateDto> rateDtoValidator, IExchangeRateMapper mapper, IOptions<ExchangeRateProviderOptions> options)
        {
        ...
        public async Task<IEnumerable<ExchangeRate>> GetExchangeRates(IEnumerable<Currency> currencies)
        {
            ...
            var response = JsonSerializer.Deserialize<CnbRateDto>(data, jsonSerializerOptions);

            var validationResult = _rateDtoValidator.Validate(response);
            if (!validationResult.IsValid)
            {
                var errorMessages = string.Join(", ", validationResult.Errors.Select(e => e.ErrorMessage));
                throw new Exception("Data validation failed: " + errorMessages);
            }
