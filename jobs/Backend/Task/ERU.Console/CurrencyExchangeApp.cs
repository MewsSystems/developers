...
internal class CurrencyExchangeApp : IHostedService
{
	...
	{
		_logger.Log(LogLevel.Information, "Starting currency exchange rate app. Ctrl-C to end");
	...
	public async Task StopAsync(CancellationToken cancellationToken)
	{
		...
		_logger.Log(LogLevel.Information, "Stopping currency exchange rate app");
	}
	...
	private async Task RetrieveAndOutputExchangeRates(CancellationToken cancellationToken)
	{
		try
		{
			...
			var rates = (await TryGetExchangeRates(_currencyExchangeProvider, Currencies, cancellationTokenSource.Token)).ToList();
	...
	private static async Task<IEnumerable<ExchangeRate>> TryGetExchangeRates(IExchangeRateProvider exchangeRateProvider, IEnumerable<Currency> currencies,
