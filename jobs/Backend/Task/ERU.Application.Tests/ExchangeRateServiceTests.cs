...
public class ExchangeRateServiceTests
{
	...
	private Mock<IContractObjectMapper<CnbExchangeRateResponse, Domain.ExchangeRate>> _mapperMock;
    ...
    public void Setup()
    {
	    ...
	    _mapperMock = new Mock<IContractObjectMapper<CnbExchangeRateResponse, Domain.ExchangeRate>>();
	    _mapperMock.Setup(x => x.Map<CnbExchangeRateResponse, Domain.ExchangeRate>(It.IsAny<CnbExchangeRateResponse>()))
    ...
    public async Task ShouldReturnEmptyListWhenNoData()
    {
	    ...
	    Assert.ThrowsAsync<ArgumentException>(async () => await _service
		    .GetExchangeRates(new[] { new Currency("USD"), new Currency("EUR")}, CancellationToken.None));
	    var result = await _service
