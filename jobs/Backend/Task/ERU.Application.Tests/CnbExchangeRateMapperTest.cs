...
public class CnbExchangeRateMapperTest
{
	...
	[Test]
	[TestCaseSource(nameof(TestMapInput))]
	public void TestMap(decimal amount, string code, decimal rate, decimal expectedRate)
	{
		ExchangeRate? result = Contract.Check(new CnbExchangeRateResponse(amount, code, rate), _mapper, "CNB ExchangeRates API response contract failure.");
