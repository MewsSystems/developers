...
namespace ExchangeRateUpdater.Providers.Tests.ProviderStrategies
{
    public class CzechNationalBankExchangeRateProviderTests : MoqMeUp<CzechNationalBankExchangeRateProvider>
    {
        ...
        [Fact]
        public async Task GetExchangeRates_DefaultBehaviour_ReturnsCorrectStrategy()
        {
            // Arrange
            ...
            var act = await target.GetExchangeRates(new List<Currency> { new Currency("EUR"), new Currency("DUMMY") });
