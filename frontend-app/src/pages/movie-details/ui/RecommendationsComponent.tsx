...
export function RecommendationsComponent({ detailsProps }: { detailsProps: DetailsProps }) {
    const navigate = useNavigate();
    const backdropSizePosition = 0;
    const backdropWidth = parseWidth(detailsProps.configuration.images.backdrop_sizes[backdropSizePosition]);
        ...
        {recommendations.map(c => {
            return (<Box onClick={() => {
                ...
                <HorizontalItem key={c.id} width={backdropWidth}>
                    <WithFallbackImage width={backdropWidth} src={parseBackdropToImgPath(detailsProps.configuration.images, c.backdrop_path, backdropSizePosition)} />
                </HorizontalItem>
                <Box p="2">
                    <Text>{c.title}</Text>
