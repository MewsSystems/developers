...
test("rootReducer should return initialState", () => {
  // $FlowTest
  const state = rootReducer(undefined, {});
  expect(state).toEqual(initalState);
});

test("rootReducer should handle CONFIGS_FETCH_START action", () => {
  const state = rootReducer(initalState, { type: "CONFIGS_FETCH_START" });
  expect(state.isFetchingConfig).toEqual(true);
});

test("rootReducer should handle CONFIG_FETCH_SUCCESS action", () => {
...
test("rootReducer should handle RATES_FETCH_START action", () => {
  const state = rootReducer(initalState, {
...
test("rootReducer should handle RATES_FETCH_SUCCESS action", () => {
  let state;
  state = rootReducer(initalState, {
...
test("rootReducer should handle CONFIGS_FETCH_FAIL action", () => {
  const state = rootReducer(initalState, {
...
test("rootReducer should handle SELECT_RATES_IDS action", () => {
  let state;
  state = rootReducer(initalState, {
