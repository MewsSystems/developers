...
RSpec.describe ExchangeRateErrorHandler, type: :controller do
  # Create a test controller to test the concern
  controller(ApplicationController) do
    include ExchangeRateErrorHandler

    def index
      raise ExchangeRateErrors::ValidationError.new("Invalid currency")
    end

    def show
      raise ExchangeRateErrors::ProviderAuthenticationError.new("Auth failed")
    end

    def update
      raise ExchangeRateErrors::ProviderUnavailableError.new("Provider down for maintenance")
    end
  end
