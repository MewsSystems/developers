...
const MovieDetailInformation = ({ ...props }: MovieDetailInformationProps) => {
    ...
    return (
        <>
            ...
            >
                <Divider />
                <Typography data-testid="movie-genre" variant="body2">
                    {genres?.map(({ name }) => name).join(' / ')}
                </Typography>
                <Divider />
                <Typography data-testid="movie-release" variant="body2">
                    {new Date(releaseDate).getFullYear()}
                </Typography>
                <Divider />
                <Typography data-testid="movie-origin" variant="body2">
                    Origins: {originalTitle} (
                    {originalLanguage &&
                        languageNameInEnglish.of(originalLanguage)}
                    )
                </Typography>
            </Box>
        </>
